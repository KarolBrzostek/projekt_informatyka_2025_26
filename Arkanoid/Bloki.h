...
public:
	...
	int sprawdzKolizje(Pilka& pilka);
	void usunZniszczoneBloki();
	const std::vector<Cegla>& getBloki() const;

	void odbuduj(const std::vector<BlockData>& dane, float szerOkna, float wysOkna);
...
int Bloki::sprawdzKolizje(Pilka& pilka)
{
	int punktyZdobyte = 0;
...
const std::vector<Cegla>& Bloki::getBloki() const
{
	return bloki;
}

void Bloki::odbuduj(const std::vector<BlockData>& dane, float szerOkna, float wysOkna)
{
	rozmiarX = ((szerOkna - (LICZBA_KOLUMN - 1) * 2.f) / LICZBA_KOLUMN);
	rozmiarY = 20.f;

	bloki.clear();
	for (auto& d : dane)
	{
		bloki.emplace_back(sf::Vector2f(d.x, d.y), sf::Vector2f(rozmiarX, rozmiarY), d.hp);
	}
}